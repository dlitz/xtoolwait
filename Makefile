...
# operating system:  Linux 3.17.1-1-ARCH x86_64 [ELF] (3.17.1)
# libc:	(6.20.0)
# binutils:	(224)
...
xtoolwait.$(MANNEWSUFFIX): xtoolwait.$(MANSRCSUFFIX)
	...
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   ...
	   $(LN) `basename xtoolwait.$(MANSRCSUFFIX)` `basename $@`; \
	     ...
	     < xtoolwait.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi
